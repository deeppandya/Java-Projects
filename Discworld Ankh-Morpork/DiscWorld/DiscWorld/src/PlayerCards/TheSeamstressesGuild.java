 ...
 * 
 * @author p_pandy
 * <p> Player card,from Green cards, The Seamstresse's Guild extends the PlayerCardActions.
...
public class TheSeamstressesGuild extends PlayerCardActions {
	
	//scroll
	// place a minion
	public TheSeamstressesGuild(String name) {
		playerCardName=name;
	...
	public void scroll() {
		...
		if(Helper.payToBank(2, temp)){
			...
			do
			{
				...
				if(num>=0 && p.playerCards.contains(p.playerCards.get(num)) && !(p.playerCards.get(num).used))
				{
					p.playerCards.get(num).used=true;
					temp.playerCards.add(p.playerCards.get(num));
					break;
				}
				else
				{
